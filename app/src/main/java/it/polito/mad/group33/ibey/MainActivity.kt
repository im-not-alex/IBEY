...
//    }
    ...
    private fun getUserImage(name:String, location:String){
        getImg(name,location).addOnSuccessListener {
            Thread(Runnable {
                Glide.with(applicationContext).asBitmap().load(it).error(R.mipmap.ic_launcher_round).into(object : CustomTarget<Bitmap>(){
