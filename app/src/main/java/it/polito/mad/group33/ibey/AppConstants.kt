...
fun getAddressFromCoordinates(latitude: Double, longitude: Double, context: Context, onlyCity: Boolean =false) : String
{
    ...
    if (onlyCity)
        return (if(address.locality!=null) address.locality + ", " else "") + (if(address.subAdminArea!=null) address.subAdminArea +", " else "") + address.countryName
...
fun updateLocationMarker(location: Pair<Double, Double>, googleMap: GoogleMap, textLocation: TextInputEditText, context: Context, marker: Marker) : Marker
{
    val latLng = LatLng(location.first, location.second)
...
fun myLocationEnabled(googleMap: GoogleMap, activity: Activity, setCurrentLocationOnMap: (location: Location) -> Unit)
{
