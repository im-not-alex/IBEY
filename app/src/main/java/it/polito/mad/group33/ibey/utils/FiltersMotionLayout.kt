package it.polito.mad.group33.ibey.utils
...
class FiltersMotionLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    //private val durationsMap: Map<MotionScene.Transition, Int> = definedTransitions.associateWith { it.duration } TODO NOT NEEDED
    ...
    private fun openSheet(): Unit = performAnimation {
        ...
        //startScaleDownAnimator(true) TODO
    ...
    private fun onFilterApplied(): Unit = performAnimation {
       ...
       // (context as MainActivity).isAdapterFiltered = true TODO SET IN FRAGMENT
        ...
        // Remove adapters
       // tabsHandler.setAdapters(false) TODO
    }
    ...
    private fun unFilterAdapterItems(): Unit = performAnimation {
        ...
        //(context as MainActivity).isAdapterFiltered = false  TODO IN FRAGMENT
    ...
    private fun enableClicks() = when (currentState) {
        R.id.set1_base, R.id.set10_unfilterOutset -> {
            filterIcon.setOnClickListener {
                findFragment<OnSaleListFragment>().activity?.hideKeyboard()
                openSheet()
            }
        ...
        R.id.set4_settle -> {
            filterIcon.setOnClickListener {
                findFragment<OnSaleListFragment>().manageFilterQueryWithPriceExtraction()
                findFragment<OnSaleListFragment>().activity?.hideKeyboard()
                onFilterApplied() }
        ...
        R.id.set7_filterBase -> {
            closeIcon.setOnClickListener {
                findFragment<OnSaleListFragment>().clearFilterAndExecuteQuery()
                findFragment<OnSaleListFragment>().activity?.hideKeyboard()
    ...
    private fun startScaleDownAnimator(isScaledDown: Boolean): Unit =
            (context as MainActivity) //todo not main activity
